...
class LoadFromImage:
    # YOLOv5 image loader, i.e. python detect.py --source image.jpg
    def __init__(self, img, img_size=640, stride=32, auto=True):
        ...
        # If input is a path string, read the image
        if isinstance(img, str):
            img = cv2.imread(img)  # BGR
            if img is None:
                raise FileNotFoundError(f"Image not found at {img}")
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  # Convert to RGB
        # If input is already an image array (assumed to be RGB)
        elif isinstance(img, np.ndarray):
            pass
        else:
            raise TypeError("Input must be either image path (str) or numpy array")

        self.img0 = img  # Original image
        self.imgs = [img]  # List containing the image
        self.sources = ['image']  # Source name

        # Check for common shapes
        s = np.stack([letterbox(x, self.img_size, stride=self.stride, auto=self.auto)[0].shape
                      for x in self.imgs])
    ...
    def __next__(self):
        ...
        self.count += 1
        return self.sources[0], img, img0, None, ''
