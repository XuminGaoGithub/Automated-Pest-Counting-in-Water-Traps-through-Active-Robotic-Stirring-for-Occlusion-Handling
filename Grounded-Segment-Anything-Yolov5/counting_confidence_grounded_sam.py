'''
python counting_confidence_grounded_sam.py --config GroundingDINO/groundingdino/config/GroundingDINO_SwinT_OGC.py --grounded_checkpoint /home/xumin/Open-GroundingDino/weights/groundingdino_swint_ogc.pth --sam_hq_checkpoint /home/newdrive/summerschool/Grounded-Segment-Anything/sam_hq_vit_b.pth --use_sam_hq  --input_dir ./counting_confidence/src --output_dir "./simulation_output/image_processing" --box_threshold 0.5 --text_threshold 0.25 --device "cuda"
...
4.直接读取图像进行处理，读取图像之后不保存了
5.#img, mask_stirring_tool = save_output_with_opencv(image_cv, largest_mask, filtered_boxes_crop, filtered_phrases, output_image_path)
6.计算各个处理节点run_time
'''
...
def process_directory(input_dir, output_dir, args):
    ...
    while True:
        ...
        start_time_read_2 = time.time()
        time_read = start_time_read_2 - start_time_read_1
        ...
        print("Time_records:",time_resize)

        #image_name = f"{base_name}_{frame_index}"
        ...
        if len(yellow_pan_boxes) == 0:
            ...
            start_time_crop_1 = time.time()
            crop_img = cv2.cvtColor(crop_img, cv2.COLOR_RGB2BGR)
            cv2.imwrite(output_crop_path, crop_img)
            start_time_crop_2 = time.time()
            time_crop = start_time_crop_2 - start_time_crop_1
            print("Time_crop:", time_crop)
        ...
        if len(filtered_boxes) == 0:
            ...
            if len(filtered_centroids) > 1:
                ...
                print("Time_distribution:", time_distribution)
