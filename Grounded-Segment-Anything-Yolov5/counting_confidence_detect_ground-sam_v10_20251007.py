'''
python counting_confidence_detect_ground-sam_v10_20251007.py --config GroundingDINO/groundingdino/config/GroundingDINO_SwinT_OGC.py --grounded_checkpoint /home/xumin/Open-GroundingDino/weights/groundingdino_swint_ogc.pth --sam_hq_checkpoint /home/newdrive/summerschool/Grounded-Segment-Anything/sam_hq_vit_b.pth --use_sam_hq  --input_dir ./counting_confidence/src --output_dir "./simulation_output/image_processing" --box_threshold 0.5 --text_threshold 0.25 --device "cuda" --weights /home/newdrive/Phd/Modification_yolov5/runs/train/voc2060_yolov5s-ODConv-cotnet2/weights/best.pt --device_yolo 0 --save-txt --save-conf --source ./counting_confidence/yellow_pan
...
# python counting_confidence_detect_ground-sam_v10_20251007.py.py在counting_confidence_detect_ground-sam_v10_20250903.py版本上
#(1)开启保存检测yellow_pan的function #def save_output_with_opencv_boxes_(original_frame, boxes, phrases, output_path):
#(2)添加了image_time_records.append(image_time) #图像采集点cpature
#(3)  if count<1:时 time.sleep(1) #防止马上结束之后进入下一个循环,导致第一frame和第二frame的images无差别
#(4) # 调节constatnt_adaptive speed
                    #speed = max(0.1, min(0.55, speed)) #adaptive speed
...
cv2.imwrite(output_crop_path, crop_img) 不要#
...
# 3. 定义多项式回归训练和测试函数
def train_and_test_poly_model(X_train, y_train, X_test, y_test, condition_name):
    results = {}
    ...
    for model_name, metrics in results.items():
        print(f"{model_name}:")
        print(f"  Train MSE: {metrics['Train MSE']}, Train R^2: {metrics['Train R^2']}")
        print(f"  Test MSE: {metrics['Test MSE']}, Test R^2: {metrics['Test R^2']}")
    print("\n")
...
class AphidAnalyzer:
    ...
    def _basic_analysis(self):
        ...
        print("\n===== Basic Statistics =====")
        print(f"Total objects: {len(self.centroids)}")
...
# ====== 核心函数 ======
def compute_delta_total(sliding_window, polynomial_models):
    """
    计算滑动窗口内reliability的平均线性变化率
    参数:
        sliding_window: 包含k帧指标的列表，每帧是包含各指标的字典
        polynomial_models: 包含训练好的多项式模型的字典
    返回:
        平均线性变化率 Δ = 1/(k-1) * Σ(reliability[i+1] - reliability[i])
    异常:
        当缺少必要特征时抛出KeyError
        当窗口帧数不足2时抛出ValueError
    """
    # 1. 验证模型完整性
    try:
        model = polynomial_models['Polynomial Regression (Degree 2)']['Model']
        poly = polynomial_models['Polynomial Regression (Degree 2)']['Poly Transformer']
    except KeyError as e:
        raise ValueError("模型结构不完整，缺少必要组件") from e

    # 2. 检查窗口大小
    if len(sliding_window) < 2:
        raise ValueError("滑动窗口至少需要2帧数据")

    # 3. 计算各帧reliability（严格模式）
    reliabilities = []
    for frame in sliding_window:
        ...
        features = np.array([
            ...
            frame['IC'],  # IC_Score
            frame['PDU'],  # PDU_Score
            frame['MeanConfidence'],  # mean_confidence_value
            frame['Detections'],  # len_det
            frame['Blurriness']  # blurriness
        ]).reshape(1, -1)

        # 预测reliability
        features_poly = poly.transform(features)
        reliability = model.predict(features_poly)[0]
        predicted_reliability = sigmoid(reliability)
        print('predicted_reliability:',predicted_reliability)
        reliabilities.append(predicted_reliability)

    # 4. 计算平均线性变化率
    delta_sum = sum(
        (reliabilities[i + 1] - reliabilities[i])
        for i in range(len(reliabilities) - 1)
    )

    average_delta = delta_sum / (len(reliabilities) - 1)

    return average_delta,reliabilities
...
def process_directory(input_dir, output_dir, args):
    ...
    # 1. 数据整合
    # 读取niqe.csv
    niqe_df = pd.read_csv('niqe.csv')
    niqe_df.columns = ['Image Name', 'IQA_Score']

    # 读取entropy.csv
    entropy_df = pd.read_csv('entropy.csv')
    entropy_df.columns = ['Image Name', 'IC_Score']

    # 读取dbscan.txt
    dbscan_df = pd.read_csv('dbscan.txt', sep=',', header=None)
    dbscan_df.columns = ['Image Name', 'Method', 'PDU_Score']
    dbscan_df = dbscan_df[['Image Name', 'PDU_Score']]

    # 读取sorted_CNBR_soft_nms.txt
    detection_df = pd.read_csv('sorted_CNBR_soft_nms.txt', sep=',', header=None)
    detection_df.columns = ['Image Name', 'TP', 'FP', 'FN', 'Counting_Confidence']

    # 读取cnbr.txt
    cnbr_df = pd.read_csv('cnbr.txt', sep=',', header=None)
    cnbr_df.columns = ['Image Name', 'mean_confidence_value', 'len_det', 'blurriness', 'Realiability']

    # 合并数据
    df = pd.merge(niqe_df, entropy_df, on='Image Name')
    df = pd.merge(df, dbscan_df, on='Image Name')
    df = pd.merge(df, detection_df, on='Image Name')
    df = pd.merge(df, cnbr_df, on='Image Name')

    # 2. 数据划分
    train_df, test_df = train_test_split(df, test_size=0.3, random_state=42)

    # 4. 只计算特定条件的多项式回归结果
    condition_name = "IQA_Score, IC_Score, PDU_Score, mean_confidence_value, len_det, blurriness vs Realiability"
    X_train = train_df[['IQA_Score', 'IC_Score', 'PDU_Score', 'mean_confidence_value', 'len_det', 'blurriness']]
    y_train = train_df['Realiability']
    X_test = test_df[['IQA_Score', 'IC_Score', 'PDU_Score', 'mean_confidence_value', 'len_det', 'blurriness']]
    y_test = test_df['Realiability']

    # 训练和测试多项式回归模型
    polynomial_models = train_and_test_poly_model(X_train, y_train, X_test, y_test, condition_name)

    # 5. 解释多项式回归的训练过程
    print("多项式回归（二次项）的训练过程解释：")
    print("1. 首先使用PolynomialFeatures(degree=2)将原始6维特征转换为二次多项式特征")
    print("   - 这包括所有原始特征、它们的平方项以及所有特征的两两交互项")
    print("   - 例如：对于特征x1和x2，会生成x1, x2, x1^2, x2^2, x1*x2")
    print("2. 然后对生成的多项式特征使用StandardScaler进行标准化（默认包含在LinearRegression中）")
    print("3. 最后使用线性回归模型拟合这些多项式特征与目标变量之间的关系")
    print("4. 模型通过最小化均方误差(MSE)来优化系数")
    print("\n")
##////////////////////////////////////////////////////////////##
    ...
    k = 3
    speed = 0.5 #初始速度
    adjust_factor = 1 #1 #1 #0.1  # 控制调节幅度（建议 0.1~0.3）
    stop_threshold = 0.01
    max_speed = 0.5
    min_speed = 0.1
...
###########################################################################
    ...
    # 1. Video #
    '''
    video_source = "/home/xumin/Music/111/low_four_circles_1.avi"  #视频
    ...
    # 如果没有设置 BUFFERSIZE，OpenCV 默认会积涨旧帧
    # 设置 BUFFERSIZE， OpenCV只保留最后 1 帧，也就是说：你每次 cap.read() 得到的图像都是当前时刻摄像头最新捕获的帧
    cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
    ret = cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
    print("Setting BUFFERSIZE result：", ret)  # True 表示成功，False 表示无效


    time_records=[]
    image_time_records=[]
    frame_index = 0
    video_source_dir = "/home/xumin/Music/111/low_four_circles_1.avi"


    # 1. Camera_逐帧处理 or 即时处理 ret = cap.set(cv2.CAP_PROP_BUFFERSIZE, 1) #
    ##设置摄像头为高分辨率
    ...
    cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG')) # 强制设置 MJPEG 编码（关键步骤）
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280) # 设置高分辨率
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
    width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    print(f"[INFO] Actual size of images: {width}x{height}") # 检查是否真正生效
...
###------------ 1.yellow_pan detection by grounded_dino --------------------------###
        ...
        if len(yellow_pan_boxes) == 0:
            ...
            # 记录图像采集点时间
            end_time = time.time()
            image_time = end_time - Total_start_time
...
#------------------------------------------------------------------------#
        ...
        # 如果检测到多个 "wood stick"，根据得分（logit）只保留得分最高的那个
        logits_filt = logits.clone()
        score_per_box = logits_filt.max(dim=1)[0]  # 每个过滤后的框的最大得分
        wood_stick_candidates = []
        for box, phrase, score in zip(boxes_filt, pred_phrases, score_per_box):
            if 'wood stick' in phrase:
                wood_stick_candidates.append((box, phrase, score.item()))  # .item() 得到 float 值
        # print('wood_stick_candidates:',wood_stick_candidates)

        if len(wood_stick_candidates) == 1:
            box, phrase, _ = wood_stick_candidates[0]
            filtered_boxes.append(box)
            filtered_phrases.append(phrase)
        elif len(wood_stick_candidates) > 1:
            best = max(wood_stick_candidates, key=lambda x: x[2])  # 按照 score 排序
            box, phrase, _ = best
            filtered_boxes.append(box)
            filtered_phrases.append(phrase)
        # print('filtered_phrases:',filtered_phrases)
...
#-----------------------###3. 执行Yolov5检测 ###----------------------------##
            ...
            image_name = f"frame_{frame_index:06d}.jpg"
            # 去掉视频文件名，保留目录部分
            video_dir = os.path.dirname(video_source_dir)  # /home/newdrive/simulation/test/test_20241120/video
            # 拼接成完整的路径
...
#----------------------------------------------------------------#
    ...
    #-----------------------  ### 3. 执行Yolov5检测 ###----------------------------#
            ...
            ## 4. 图像处理 ##
            mask_stirring_tool = None
            filtered_centroids, aphid_size = analysis(img, image_name, pred, output_folder_color,
...
#####################perception<->speed###############################################
        #print("mean_confidence_value:",mean_confidence_value)
        #防止mean_confidence_value = mean_confidence.item()导致nan
        if predict_number==0:
            mean_confidence_value=0
        ...
        if count<1:
            print("[INFO] no processing for First frame")
            First_frame_end_time = time.time()
            First_frame_time = First_frame_end_time - Total_start_time
            print('First_frame_time:',First_frame_time)

            packed_data = struct.pack('<f', speed)  # 小端 float
            sock.sendto(packed_data, (UDP_IP, UDP_PORT))

            time.sleep(1) #防止马上结束之后进入下一个循环,导致第一frame和第二frame的images无差别
            ...
            if len(history) >= k:
                ...
                delta_total,reliabilities = compute_delta_total(sliding_window, polynomial_models)
                change_total.append(delta_total)
                reliabilities_group.append(reliabilities)
                ...
                if abs(delta_total) < stop_threshold:
                    ...
                    Total_time = Total_time-First_frame_time
                    print('Total_time:',Total_time)
                    result.write("time_records: " + str(time_records) + "\n")
                    result.write("Total_time: " + str(Total_time) + '\n')
                    result.write("reliabilities: " + str(reliabilities_group) + "\n")
                    result.write("change_total: " + str(change_total) + "\n")
                    result.write("speed_records: " + str(speed_records) + "\n")
                    result.close()
                    ...
                    packed_data = struct.pack('<f', speed)  # 小端 float
                    sock.sendto(packed_data, (UDP_IP, UDP_PORT))
                    print(f"Sent speed = {speed:.3f} to {UDP_IP}:{UDP_PORT}\n")

                    image_time_records.append(image_time)
                    print("image_time_records:", image_time_records)
                    ...
                    # 加入上下限限制
                    #speed = max(0.1, min(0.59, speed))

                    #constant speed
                    speed = 0.5

                    print(f" → update speed: speed = {speed:.4f}")
