...
# python counting_confidence_detect_ground-sam_v11.py在v9版本上,yolov5部分添加了confusionmatrix,记录TP,FP,FN
...
###########################################################################
    ...
    # 参数设置
    video_folder = "/home/xumin/Music/111"  # 包含多个 .avi 视频的文件夹

    # 遍历所有avi视频文件
    video_files = sorted(glob.glob(os.path.join(video_folder, "*.avi")))
    print(f"[INFO] Find {len(video_files)} video file。")
    ...
    for video_path in video_files:
        ...
        while frame_index < 25:
            ...
            if frame_index == 0:
                ms_pos = 100  # 第0帧：跳过第0秒(image invalid at 0s sometimes)，读取100ms秒处图像
            else:
           ...
           #print("time_read:", time_read)
            ...
            image_name = f"{video_id}_frame_{frame_index:06d}.jpg"
            print('image_name:',image_name)
                ...
                if crop_img is not None:
                    ...
                    for path, im, im0s, vid_cap, s in dataset:
                        ...
                        for i, det in enumerate(pred):  # per image
                            ...
                            # xml_path = '/home/xumin/yolov5/dataset/voc2011_1_53_0/Annotations/1_53_0.xml'
                            xml = args.xml
                            ...
                            fp = 0
                            fn = 0
                            gt_real = 6  # gt number of aphids in the yellow pan
                            print('gt_real:', gt_real)

                            if (pred_copy.numel() != 0) and (label.numel() != 0):
                                confusion_matrix = ConfusionMatrix(nc=nc)
                                confusion_matrix.process_batch(pred_copy, label)
                                # for single image
                                # confusion_matrix.plot_with_name(normalize=False, save_dir=save_dir, names=list(names.values()))
                                confusion_matrix.plot_with_name(normalize=False, save_dir=save_dir,
                            ...
                            cnbr.write(file_name + '.jpg' + ',' + str(mean_confidence_value) + ',' + str(
                                len(det)) + ',' + str(
                                blurriness) + ',' + str(Realiability) + ',' + str(tp) + ',' + str(
                                fp) + ',' + str(fn) + ',' + str(gt_image) + ',' + str(gt_real) + '\n')
    ...
    #----------------------------------------------------------------#
            ...
            if len(filtered_boxes) != 0:
                ...
                #print ('len(adjusted_boxes):',len(adjusted_boxes))
