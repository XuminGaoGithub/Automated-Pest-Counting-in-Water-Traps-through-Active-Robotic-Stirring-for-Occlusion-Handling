'''
python counting_confidence_detect_ground-sam_v6.py --config GroundingDINO/groundingdino/config/GroundingDINO_SwinT_OGC.py --grounded_checkpoint /home/xumin/Open-GroundingDino/weights/groundingdino_swint_ogc.pth --sam_hq_checkpoint /home/newdrive/summerschool/Grounded-Segment-Anything/sam_hq_vit_b.pth --use_sam_hq  --input_dir ./counting_confidence/src --output_dir "./simulation_output/image_processing" --box_threshold 0.5 --text_threshold 0.25 --device "cuda" --weights /home/newdrive/Phd/Modification_yolov5/runs/train/voc2060_yolov5s-ODConv-cotnet2/weights/best.pt --device_yolo 0 --save-txt --save-conf --source ./counting_confidence/yellow_pan
...
##////////////////////////////////////////////////////////////##
    ...
    k = 3
    speed = 0.5 #初始速度
    adjust_factor = 0.5 #1 #0.1  # 控制调节幅度（建议 0.1~0.3）
    stop_threshold = 0.08
    max_speed = 0.8
    min_speed = 0.1
...
###########################################################################
    ...
    # Video #
    video_source = "/home/xumin/Music/111/med_round_3.avi"  #视频
    ...
    video_source_dir = "/home/newdrive/simulation/test/test_20241120/video/Aphid_50.mp4"
    interval_sec = 2  # 间隔2秒
    time_records=[]
    frame_index = 0
    ...
    """
    # Camera_逐帧处理
    # or 即时处理 ret = cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
    """
    #设置摄像头为高分辨率
    ...
    #ZED-camera,只取一半
    # '''
    # cap.set(cv2.CAP_PROP_FRAME_WIDTH, 2560)  # 左右拼接后的总宽度
    ...
    #     # 获取图像宽度的一半（分离左右图像）
    #     height, width = frame.shape[:2]
    #     left_image = frame[:, :width // 2]  # 左眼图像
    #     frame = left_image
    #     # right_image = frame[:, width//2:]  # 可选：右眼图像
    # '''
...
#####################perception<->speed###############################################
        ...
        if len(history) >= k:
            ...
            delta_total = compute_delta_total(sliding_window, polynomial_models)
